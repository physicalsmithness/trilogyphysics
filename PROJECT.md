...
- **Docs and code (this repo):** `C:\Claude (not on Gdrive, nor OneDrive)\Trilogy Physics`, a flat repo root on the non-sync drive (DECISIONS d012 (home moved to the non-sync drive), superseding the provisional G: home in d010). `.git` and the GitHub remote (github.com/physicalsmithness/trilogyphysics) carried across; GitHub is the backsto