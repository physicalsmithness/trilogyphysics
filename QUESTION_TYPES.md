...
# Waves 6.6 extensions (vocabulary RATIFIED d043, 2026-06-11; bank authored, app/topics/waves_6_6.js)

Builds on the qtype table and the d038 calc tiers. What 6.6 adds or stresses:

- **`qtype:"widget"` carries real items for the first time** (d031/d035: many 6.6 items ARE the widget): mark-a-wavelength/amplitude on `wave_train` (2 marks, method+value), drag-the-refracted-ray on `refraction_ray` (direction + angle tolerance). Authored now, flagged `pending_engine:"widget_qtype"` until Housing wires the submit flow. Widget scorer errorCodes are registered misconception slugs (one taxonomy).
- **Calc profile is the inverse of Forces**: single-formula (21 vs 2 multi-stage in the tagged bank) with the difficulty in Layers 3-4 — prefixes (kHz, GHz, ms, and uniquely nm on OUTPUT) and standard form both ways. Until the grader grades prefix conversion and standard-form output, these are `interim_for:"calc_prefix"` MCQs. v=f-lambda and T=1/f grade full as calc_workings (lambda authored as single-char L). The d037 chains (v=f-lambda -> v=d/t; f=1/T first; echo d=2x; radar) are GAP-PASS items, authored as enriched MCQs with stages/markScheme/failsAt (the 6.5 batch-7 pattern).
- **Tier-2 prep-step calcs (d038) are the bank's signature**: graph read -> v=f-lambda (the sound-temperature shape), table means -> convert cm -> v=f-lambda. Authored with a leading read/mean stage so the prep is separately diagnosable (`graph_readoff_left_as_answer`, now shared WS).
- **Method 4/6-markers are the topic's worst scorers** (ripple tank, speed of sound by echo and direct, string RP, RP21: 9-25% on real papers) and fail two ways every year, encoded as wrong claims in claim-point `short` items: `apparatus_described_not_method` and `measurement_purpose_missing`.
- **Definition families mark on TWO co-present elements** (oscillations AND perpendicular/parallel-to-energy-transfer): keyword `short` mis-grades them, so they are MCQ/claim-points (d034 steer honoured).
- **Families**: wave anatomy classify/define/label; quantity definitions with named-trap distractors (amplitude_peak_to_trough, wavelength_half_marked); v=f-lambda three ways + qualitative; measurement methods; refraction (name/direction/normal FH, speed-cause + wavefront explanation HT, wavelength-dependent behaviour HT); EM order/ends/inverse pairing; HT radio-oscillation origins (the catastrophic 1-10% cluster); dangers (ionising-vs-radioactive, dose data); uses + HT suitability; infrared RP (surface rankings, rate-vs-final-temp, thermal equilibrium).
- **Out of scope** (d043): sound-in-solids/ear, ultrasound-as-topic, TIR, lenses/colour/black-body. Echo calcs stay, in Trilogy-safe frames (clap-wall, radar).
