...
## 2026-06-11, from Architecture: waves bank needs a shape migration before it can run
Your 6.6 content is complete and ratified (d043/d048), but waves_6_6.js is in the early item shape (mcq_single/distractors/status/tier:"both") which the engine does not read - it would render blank. It needs migrating to the locked engine JSON shape (qtype/choices/answerIndex/tier F|H|FH/misconception_id/atoms[]/syllabus_codes[]), exactly as the Forces author did (review/forces_6_5_migration_and_enhancement.md has the transform). Either run that transform on your file or coordinate with Housing; the topic stays disabled until it conforms (d053). Author new items in the engine shape from here. No content change needed, just the shape.

## 2026-06-17, from Authoring 6.6: engine-shape migration DONE (d053)
Migrated all 109 items to the locked engine JSON shape via a Node transform on the real objects (the Forces precedent, review/forces_6_5_migration_and_enhancement.md): mcq_single->mcq with choices[]+answerIndex and per-choice misconception_id; mcq_multi->answerIndices; claim `short`->level_of_response_6 with lor.points[{text,creditworthy,misconception_id}] and marks=creditworthy count (so the gradeLor default gives 1 mark/point - this corrected 8 items that had carried their AQA 4-mark total against 3 creditworthy points; the true AQA totals stay in the review notes + aqa_ppq source refs); calc_workings and the 3 qtype:"widget" items kept their blocks; tier both/foundation/higher -> FH/F/H; added board/topic/atoms[]/syllabus_codes[]/applicable_misconceptions[]/source. Enriched d037 chains stay mcq carrying calc.stages/markScheme/failsAt.

VERIFIED: loads; qtype counts 86 mcq / 5 mcq_multi / 10 level_of_response_6 / 5 calc_workings / 3 widget; a cross-check against the pre-migration backup confirms EVERY answer key preserved (each answerIndex/answerIndices points to the originally-correct option text; every lor creditworthy set matches). Backup at outputs/waves_6_6.premigration.bak.js. Topic now conforms to d053; ready for Housing to wire into app/index.html and enable. Did NOT add the d036 picture-MCQ items yet (holding for Smith's item feedback, small batches).

## 2026-06-19, from Authoring 6.6: real-engine smoke test PASS + reusable verify script
Beyond the structural/key-preservation check, ran the migrated bank through the engine's OWN graders headlessly (loads calc_workings.js + engine.js + waves_6_6.js, uses window.TRILOGY_ENGINE_TEST). Result: **109/109 items servable; every mcq/mcq_multi grades "correct", every level_of_response_6 grades "full" with marks == creditworthy count, every calc_workings grades 4/4 full** (markCalcWorkings with proper four-line working). The 3 qtype:"widget" items are servable; their scorers are covered by app/widgets/verify_waves_models.js, grading lands when Housing wires the widget submit flow. Persisted as `app/topics/verify_waves_6_6.js` (node, exit 1 on regression) so the next shape change can't silently break the topic. Confirms d053 conformance end to end; ready for Housing to enable.
