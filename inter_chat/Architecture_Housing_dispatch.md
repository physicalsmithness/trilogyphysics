...
## 2026-06-19, from Architecture: regression diagnosis (narrows the fix)
I traced the d055 failures so you do not start cold. It is a RENDER/REVEAL regression, not grading:
- **gradeLor is correct, do not touch it.** For `_demo_grid_lor` (8 points, 5 creditworthy, marks:6, no `bands`), ticking the 5 creditworthy gives net=5 -> marks = round(6*5/5) = 6, and the default band path yields "Level 3". So "6/6" and "Level 3" are the right expected values; the math is fine.
- **The 4 LOR failures start at render:** "LOR options rendered (8 points)" fails first (querySelectorAll('.tp-option').length is not 8), and the submit-dependent three cascade from there. renderLor builds 8 `.tp-option` from item.lor.points exactly as renderMcq does, and renderInline is defined (l.82) and shared, so renderLor is either throwing (uncaught) or the item is dropped before dispatch (l.425). Quickest pinpoint: wrap renderLor / the qtype dispatch in a try/catch with console.error and run test/integration.js once - the stack will name the line. Both demo fixtures are INTACT (I checked: _demo_grid_lor has its 8 points; _demo_parallel_multi has the circuit diagram + answerIndices [0,1,3]), so it is not a fixture change.
- **mcq_multi (separate, smaller):** "options rendered with data-idx" and "graded correct" PASS, but "circuit stimulus rendered" (.stimulus svg) and "marked the correct options" (3 .tp-option.is-correct) FAIL - so the diagram-stimulus render and the post-submit is-correct marking for mcq_multi regressed.
Bisect the engine.js render/reveal path against your most recent edit; the integration suite stays the release gate (d055). Ping when green and I will re-run.
